...
window.onload = function() {
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        label.textContent = `Blessing Fountain${i}：`;
        const select = document.createElement('select');
        select.id = `factory${i}`;
        select.name = `factory${i}`;
        select.options.add(new Option('select lv', ''));
        levelOutput.forEach(item => {
            select.options.add(new Option(`Lv ${item.level}`, item.level));
        });
...
function calculateOutput() {
    ...
    document.getElementById('total-output').textContent = `Output per hour: ${total}`;
    ...
    document.getElementById('difference').textContent = `You still lack: ${difference}`;
    ...
    document.getElementById('time-needed').textContent = `${hours} hours ${minutes} minutes needed`; // 显示小时和分钟

    const upgradeTime = new Date(Date.now() + hoursNeeded * 3600000);
    document.getElementById('upgrade-time').textContent = `Estimated upgrade time: ${upgradeTime.toLocaleString()}`;
}
